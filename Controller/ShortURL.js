...
const { promisify } = require("util");         //method which returns using a callback function to return responses in a promise object
const { getUrl } = require('./getUrl');

//Connect to redis
const redisClient = redis.createClient(
    14143,                                                            // Port/information
    "redis-14143.c301.ap-south-1-1.ec2.cloud.redislabs.com",          //a string like the cluster for connection
    { no_ready_check: true }                                          // Like  {useNewUrlParser: true})
);
redisClient.auth("F45rPClREsWg5BxVgExL1qZch4LjoQ54", function (err) {  //to complete authentication
    if (err) throw err;
});

redisClient.on("connect", async function () {         //Promise
